...
<%@ page import="com.grail.kantar.util.KantarUtils" %>
...
<% // Title of this page and breadcrumbs
    String title = "Document Repository Report Types";
...
<head>
    ...
    <content tag="pageID">document-repository-report-types</content>
    <content tag="pagehelp">
        <h3>Document Repository Report Types</h3>
    </content>
</head>

<div>
    <form action="document-repository-report-types.jsp" name="synchro-metaform" id="synchro-metaform" method="post">
        <table class="space-table" width="100%" border="0" cellpadding="0" cellspacing="0">
            <tbody>
            <tr>
                <td>
                    <p style="padding: 5px;">Drag and drop to reorder areas.</p>
                    <div id="tree-div">
                        <ul class="jstree">
                            <li id="space_1" class="">
                                ...
                                <ul id="space_1_ul" class="ui-sortable" style="display: block;">
                                    ...
                                    <%
                                        Map<Integer, String> reportTypes = KantarUtils.getKantarReportTypes();

                                        for(Integer id : reportTypes.keySet()) {
                                            String name = reportTypes.get(id);
                                    %>
                                    <li id="<%=id%>" class="main">
                                        <span id="name-<%=id%>"><%=name%></span>
                                        <div class="brands-select">
                                            <a href="javascript:void(0);" onclick="editProperty(<%=id%>)">
                                                <img src="../images/jive-icon-edit-16x16.gif" width="16" height="16" alt="Edit Report Type" border="0">
                                            </a>
                                            <a class="enable-delete-link" onclick="deleteProperty(<%=id%>)">
                                                <img src="../images/jive-icon-delete-16x16.gif" width="16" height="16" alt="Delete Report Type" border="0">
                                            </a>
                                        </div>
                                        <ul id="edit-box-<%=id%>" style="display:none">
                                            <li class="hidden-list">
                                                <div class="j-form-row">
                                                    ...
                                                    <input type="text" class="kantar-methodology-type-input" id="jive-propValue-<%=id%>" name="propValue" value="<%=name%>">
                                                </div>
            ...
            <tr id="add-box" style="display:none;">
                <td>
                    <div class="add-prop-header">Add Report Type</div>
                    <div class="j-form-row">
                        <input type="text" id="jive-propValue" name="propValue" >
                        <span id="jive-error-text error" style="display: none;">Entered report type already exists.</span>
                    </div>
                    <div>
                        <input class="add-btn" type="button" id="add-btn" value="Save" name="Save" />
                        <input class="add-cancel" class="kantar-methodology-type-input" type="button" id="cancel-btn" value="Cancel" name="Cancel" />
                    </div>
...
<script type="text/javascript">
    ...
    function deleteProperty(id) {
        ...
        if (res == true) {
            MetaFieldUtil.deleteDocumentRepositoryReportType(id, {
    ...
    function updateSortOrder()  {
        ...
        MetaFieldUtil.sortDocumentRepositoryReportTypes(orderMap, {
            callback: function(result) {
                $j("#synchro-metaform").submit();
            },
            async: false,
    ...
    $j(".save").click(function(){
        ...
        {
            MetaFieldUtil.updateDocumentRepositoryReportType(id, input.val(), {
                callback: function(result) {
                    $j("#synchro-metaform").submit();
                },
                async: false,
    ...
    $j(document).ready(function () {
        ...
        $j("#add-btn").click(function(){
            ...
            if(input.val()!=null && $j.trim(input.val())!="" && input.val().length < 250)
            {
                ...
                MetaFieldUtil.getDocumentRepositoryReportType(input.val(),{
                    callback: function(result) {
                        if(result) {

                        } else {
                            MetaFieldUtil.addDocumentRepositoryReportType(input.val(),{
                                callback: function(result) {
                                    $j("#synchro-metaform").submit();
                                },
                                async: false,
