...
<head>
    ...
    <content tag="pageID">synchro-region</content>
    <content tag="pagehelp">
        <h3>Regions</h3>       
    </content>
</head>

<form action="/admin/synchro-region.jsp" name="synchro-metaform" id="synchro-metaform" method="POST">
        ...
        <table class="space-table" width="100%" border="0" cellpadding="0" cellspacing="0">
			<tbody>
				<tr>
					<td>
						<p style="padding: 5px;">
							Drag and drop to reorder regions.</p>
						<div id="tree-div">
							<ul class="jstree">
								<li id="space_1" class="">									
									<span>Regions</span>
	...
	<tr id="add-box" style="display:none;">	
		<td>
			<div class="add-prop-header">Add Region</div>
			<div class="j-form-row">
...
function updateSortOrder()
{
	...
	MetaFieldUtil.sortRegion(orderMap, {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
$j(".jive-error-text").each(function(){ 
  ...
  {
	MetaFieldUtil.updateRegion(id,textarea.val(), arr.join(','), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
$j(".jive-error-text").each(function(){ 
  ...
  if(textarea.val()!=null && $j.trim(textarea.val())!="" && textarea.val().length < 250)
  {
	...
	MetaFieldUtil.addRegion(textarea.val(),selectedValues.join(","), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
