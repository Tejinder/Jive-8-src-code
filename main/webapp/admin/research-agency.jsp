...
<head>
    ...
    <content tag="pageID">research-agency</content>
    <content tag="pagehelp">
        <h3>Research Agency</h3>       
    </content>
</head>

<form action="/admin/research-agency.jsp" name="synchro-metaform" id="synchro-metaform" method="POST">
        ...
        <table class="space-table" width="100%" border="0" cellpadding="0" cellspacing="0">
			<tbody>
				<tr>
					<td>
						<p style="padding: 5px;">
							Drag and drop to reorder categories.</p>
						<div id="tree-div">
							<ul class="jstree">
								<li id="space_1" class="">									
	...
	<%		
		...
		for(Integer id : agencies.keySet())
		{
		String name = agencies.get(id); %>
		<li id="<%=id%>" class="main">		
		<span><%=name%></span>			
			<div class="brands-select">		
			<a href="javascript:void(0);" onclick="editProperty(<%=id%>)">
				<img src="../images/jive-icon-edit-16x16.gif" width="16" height="16" alt="Edit Category Name" border="0">
			</a>
			...
			<ul id="edit-box-<%=id%>" style="display:none">		
				...
				<li>
					...
					<label><span class="list">All Agency Groups</span></label>
					<!-- All Properties -->					
					<select size="3" name="all-properties-<%=id%>" id="all-properties-<%=id%>" multiple="yes">
					<%					 
					for(Integer groupID : agencyGroups.keySet())
					{
						Boolean include = false;
						Map<Integer, String> agenciesByGroup = groupAgencyMap.get(groupID);
						for(Integer agncy : agenciesByGroup.keySet())
						{
							if(agncy.intValue() == id.intValue())
							{
								include = true;
								break;
							}
						...
						String agencyGroupName = agencyGroups.get(groupID);
						if(include)
						{					
						%>
							<option value="<%=groupID%>" selected="true"><%=agencyGroupName%></option>
						<%
						}
						else
						{
						%>
							<option value="<%=groupID%>"><%=agencyGroupName%></option>
						<%
						}					
					}
					%>
				...
				<div class="form-select_div_brand">
					<label><span class="list">Selected Agency Groups</span></label>
	...
	<tr id="add-box" style="display:none;">	
		<td>
			<div class="add-prop-header">Add Research Agency</div>
			<div class="j-form-row">
		...
		<p class="select-region-heading">Select Research Agency Group(s)</p>		
				<div class="brands-select">
					<select id="select-properties" multiple>
				<%			
					for(Integer agencyGroupID : agencyGroups.keySet())
						{						
							String agencyGroupName = agencyGroups.get(agencyGroupID);				
							%>
							<option value="<%=agencyGroupID%>"><%=agencyGroupName%></option>
							<%
						}%>
					</select>
				</div>	
	...
	<tr>	
		<td colspan="3">
			<input type="button" id="add" name="add" value="Add Research Agency" />			
...
function deleteProperty(id)
{
	...
	if (res == true)
	  {
	  MetaFieldUtil.deleteResearchAgency(id, {
...
function updateSortOrder()
{
	...
	MetaFieldUtil.sortGrailResearchAgencyField(orderMap, {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
$j(".jive-error-text").each(function(){ 
  ...
  {
	MetaFieldUtil.updateGrailResearchAgency(id,textarea.val(), arr.join(','), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
$j(".jive-error-text").each(function(){ 
  ...
  if(textarea.val()!=null && $j.trim(textarea.val())!="" && textarea.val().length < 250)
  {
	...
	MetaFieldUtil.addGrailResearchAgency(textarea.val(),selectedValues.join(","), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
//	$j( ".ui-sortable" ).disableSelection();
	...
	/*$j(".set-default-prop").click(function(){
		...
		MetaFieldUtil.updateDefaultValue('grail.synchro.default.category',$id, {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
