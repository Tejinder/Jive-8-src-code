...
<head>
    ...
    <content tag="pageID">synchro-area</content>
    <content tag="pagehelp">
        <h3>Areas</h3>       
    </content>
</head>

<form action="/admin/synchro-area.jsp" name="synchro-metaform" id="synchro-metaform" method="POST">
	...
	<tr id="add-box" style="display:none;">	
		<td>
			<div class="add-prop-header">Add Area</div>
			<div class="j-form-row">
...
function updateSortOrder()
{
	...
	MetaFieldUtil.sortArea(orderMap, {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
$j(".jive-error-text").each(function(){ 
  ...
  {
	MetaFieldUtil.updateArea(id,textarea.val(), arr.join(','), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
$j(".jive-error-text").each(function(){ 
  ...
  if(textarea.val()!=null && $j.trim(textarea.val())!="" && textarea.val().length < 250)
  {
	...
	MetaFieldUtil.addArea(textarea.val(),selectedValues.join(","), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
