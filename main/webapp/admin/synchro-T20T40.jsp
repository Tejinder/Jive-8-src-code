...
<head>
    ...
    <content tag="pageID">synchro-T20T40</content>
    <content tag="pagehelp">
        <h3>T20/T40</h3>       
    </content>
</head>

<form action="/admin/synchro-T20T40.jsp" name="synchro-metaform" id="synchro-metaform" method="POST">
	...
	<%		
		Map<Integer, String> t20T40 = SynchroGlobal.getT20T40();
		Map<Integer, String> endmarkets = SynchroGlobal.getEndMarkets();
		Map<Integer, Map<Integer, String>>	mapping = SynchroGlobal.getT20T40EndMarketsMapping();
		
		List<Integer> endMarketsConsumed = new ArrayList<Integer> ();
		for(Integer id : t20T40.keySet())
		{
			Map<Integer, String> endmarketList = mapping.get(id);
			for(Integer endmarketID : endmarkets.keySet())
			{
				String endmarketName = endmarkets.get(endmarketID);
				if(endmarketList.containsKey(endmarketID))
				{
					endMarketsConsumed.add(endmarketID);
				}
			}
		}
		...
		for(Integer id : t20T40.keySet())
		{
		String name = t20T40.get(id);
		%>
		<li id="<%=id%>" class="main">
		<span><%=name%></span>	
			<div class="brands-select">			
			<a href="javascript:void(0);" onclick="editProperty(<%=id%>)">
				<img src="../images/jive-icon-edit-16x16.gif" width="16" height="16" alt="Edit Area Name" border="0">
			</a>
			...
			<ul id="edit-box-<%=id%>" style="display:none">		
				...
				<li>
					...
					<label><span class="list">All End Markets</span></label>
					<!-- All Properties -->					
					...
					for(Integer endmarketID : endmarkets.keySet())
					{
						...
						{				
						%>
							<option value="<%=endmarketID%>" selected="true"><%=endmarketName%></option>
						<%
						}
						else if(!endMarketsConsumed.contains(endmarketID))
						{
						%>
							<option value="<%=endmarketID%>"><%=endmarketName%></option>
						<%
						}					
					}
					%>
				...
				<div class="form-select_div_brand">
					<label><span class="list">Selected End Markets</span></label>
	...
	<tr id="add-box" style="display:none;">	
		<td>
			<div class="add-prop-header">Add T20/T40</div>
			<div class="j-form-row">
		...
		<p class="select-region-heading">Select End Markets</p>		
				<div class="brands-select">
					<select id="select-properties">
				<%			
					for(Integer endmarketID : endmarkets.keySet())
						{	
							if(!endMarketsConsumed.contains(endmarketID))
							{
								String endmarketName = endmarkets.get(endmarketID);				
								%>
								<option value="<%=endmarketID%>"><%=endmarketName%></option>
	...
	<tr>	
		<td colspan="3">
			<input type="button" id="add" name="add" value="Add Area" />			
...
function updateSortOrder()
{
	...
	MetaFieldUtil.sortT20T40(orderMap, {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
$j(".jive-error-text").each(function(){ 
  ...
  {
	MetaFieldUtil.updateT20T40(id,textarea.val(), arr.join(','), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
$j(".jive-error-text").each(function(){ 
  ...
  if(textarea.val()!=null && $j.trim(textarea.val())!="" && textarea.val().length < 250)
  {
	...
	MetaFieldUtil.addT20T40(textarea.val(),selectedValues.join(","), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
