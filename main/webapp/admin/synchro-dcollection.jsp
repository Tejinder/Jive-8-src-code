...
<head>
    ...
    <content tag="pageID">data-collection</content>
    <content tag="pagehelp">
        <h3>Data Collections</h3>       
    </content>
</head>

<form action="/admin/synchro-dcollection.jsp" name="synchro-metaform" id="synchro-metaform" method="POST">
        ...
        <table class="space-table" width="100%" border="0" cellpadding="0" cellspacing="0">
			<tbody>
				<tr>
					<td>
						<p style="padding: 5px;">
							Drag and drop to reorder Data Collections.</p>
						<div id="tree-div">
							<ul class="jstree">
								<li id="space_1" class="">									
	...
	<%
		...
		Map<Integer, String> allTypes = SynchroGlobal.getProjectIsMapping();
		for(Integer id : dataCollections.keySet())
		{			
			String name = dataCollections.get(id);
		%>
		<li id="<%=id%>" class="main">
		<span><%=name%></span>		
			...
			<a href="javascript:void(0);" onclick="editProperty(<%=id%>)">
				<img src="../images/jive-icon-edit-16x16.gif" width="16" height="16" alt="Edit Data Collection" border="0">
			</a>
			...
			<ul id="edit-box-<%=id%>" style="display:none">		
				...
				<li>
					...
					<label><span class="list">All Methodology Types</span></label>
					<!-- All Properties -->					
					<select size="3" name="all-properties-<%=id%>" id="all-properties-<%=id%>" multiple="yes">
					<%
					for(Integer typeID : allTypes.keySet())
					{
						String typeName = allTypes.get(typeID);
						Map<Integer, String> collectionsByType = SynchroGlobal.getCollectionMapping().get(typeID);
						if(collectionsByType.containsKey(id))
						{					
						%>
							<option value="<%=typeID%>" selected="true"><%=typeName%></option>
						<%
						}
						else
						{
				...
				<div class="form-select_div_brand">
					<label><span class="list">Selected Methodology Types</span></label>
	...
	<tr id="add-box" style="display:none;">	
		<td>
			<div class="add-prop-header">Add Data Collection</div>
			<div class="j-form-row">
		...
		<p class="select-region-heading">Select Methodology Types</p>		
				<div class="brands-select">
					<select id="select-properties" multiple>
					<%
					for(Integer id : allTypes.keySet())
					{
	...
	<tr>	
		<td colspan="3">
			<input type="button" id="add" name="add" value="Add Data Collection" />			
...
function deleteProperty(id)
{
	...
	if (res == true)
	  {
	  MetaFieldUtil.deleteDataCollection(id, {
...
function updateSortOrder()
{
	...
	MetaFieldUtil.sortDataCollection(orderMap, {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
$j(".jive-error-text").each(function(){ 
  ...
  {
	MetaFieldUtil.updateDataCollection(id,textarea.val(), arr.join(','), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
$j(".jive-error-text").each(function(){ 
  ...
  if(textarea.val()!=null && $j.trim(textarea.val())!="" && textarea.val().length < 250)
  {
	...
	MetaFieldUtil.addDataCollection(textarea.val(),selectedValues.join(","), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
