...
<head>
    ...
    <content tag="pageID">synchro-product</content>
    <content tag="pagehelp">
        <h3>Categories</h3>       
    </content>
</head>

<form action="/admin/synchro-product.jsp" name="synchro-metaform" id="synchro-metaform" method="POST">
	...
	<%
		Integer defaultValue = JiveGlobals.getJiveIntProperty(SynchroConstants.SYNCHRO_DEFAULT_CATEGORY_ID, 1);
		Map<Integer, String> products = SynchroGlobal.getProductTypes();
		Map<Integer, String> brands = SynchroGlobal.getBrands(true, new Long(1));
		Map<Integer, Object[]> mapping = SynchroGlobal.getProductBrandMapping();
		...
		<span><%=name%></span>			
			...
			<ul id="edit-box-<%=id%>" style="display:none">		
				...
				<li>
					...
					<%
					 Object[] brandList = mapping.get(id);
					for(Integer brandID : brands.keySet())
					{
						String brandName = brands.get(brandID);
						if(Arrays.asList(brandList).contains(brandID))
						{					
						%>
							<option value="<%=brandID%>" selected="true"><%=brandName%></option>
						<%
						}
						else
						{
	...
	<tr id="add-box" style="display:none;">	
		<td>
			<div class="add-prop-header">Add Category</div>
			<div class="j-form-row">
		...
		<p class="select-region-heading">Select Brand</p>		
				<div class="brands-select">
	...
	<tr>	
		<td colspan="3">
			<input type="button" id="add" name="add" value="Add Category" />			
...
function deleteProperty(id)
{
	...
	if (res == true)
	  {
	  MetaFieldUtil.deleteProduct(id, {
...
function updateSortOrder()
{
	...
	MetaFieldUtil.sortProduct(orderMap, {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
$j(".jive-error-text").each(function(){ 
  ...
  {
	MetaFieldUtil.updateProduct(id,textarea.val(), arr.join(','), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
$j(".jive-error-text").each(function(){ 
  ...
  if(textarea.val()!=null && $j.trim(textarea.val())!="" && textarea.val().length < 250)
  {
	...
	MetaFieldUtil.addProduct(textarea.val(),selectedValues.join(","), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
