...
<head>
    ...
    <content tag="pageID">synchro-brand</content>
    <content tag="pagehelp">
        <h3>Brands</h3>       
    </content>
</head>

<form action="/admin/synchro-brand.jsp" name="synchro-metaform" id="synchro-metaform" method="POST">
	...
	<%
		Integer defaultValue = JiveGlobals.getJiveIntProperty(SynchroConstants.SYNCHRO_DEFAULT_BRAND_ID, 1);
		...
		<span><%=name%></span>		
			...
			<ul id="edit-box-<%=id%>" style="display:none">		
				...
				<li>
					...
					<label><span class="list">All Products</span></label>
					<!-- All Properties -->					
					<select size="3" name="all-properties-<%=id%>" id="all-properties-<%=id%>" multiple="yes">
					<%
					List<Integer> productsMapIds = new ArrayList<Integer>();
					List<MetaField> productsMap = SynchroUtils.getProductByBrandInclusion(new Long(id));
					for(MetaField field : productsMap)
					{
						productsMapIds.add(field.getId().intValue());
					}
					for(Integer productID : products.keySet())
					{
						String productName = products.get(productID);
						if(productsMapIds.contains(productID))
						{					
						%>
							<option value="<%=productID%>" selected="true"><%=productName%></option>
						<%
						}
						else
						{
				...
				<div class="form-select_div_brand">
					<label><span class="list">Selected Products</span></label>
	...
	<tr id="add-box" style="display:none;">	
		<td>
			<div class="add-prop-header">Add Brand</div>
			<div class="j-form-row">
		...
		<p class="select-region-heading">Select Category</p>		
				<div class="brands-select">
	...
	<tr>	
		<td colspan="3">
			<input type="button" id="add" name="add" value="Add Brand" />			
...
$j(".jive-error-text").each(function(){ 
  ...
  {
	MetaFieldUtil.updateBrand(id,textarea.val(), arr.join(','), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
...
$j(".jive-error-text").each(function(){ 
  ...
  if(textarea.val()!=null && $j.trim(textarea.val())!="" && textarea.val().length < 250)
  {
	...
	MetaFieldUtil.addBrand(textarea.val(),selectedValues.join(","), {
		callback: function(result) {
			$j("#synchro-metaform").submit();
		},
		async: false,
