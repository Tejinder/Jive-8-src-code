 ...
 * @version 1.0
 */
public class ProjectWaiverDAOImpl extends JiveJdbcDaoSupport implements ProjectWaiverDAO {

    private static Logger LOG = Logger.getLogger(ProjectWaiverDAOImpl.class);
    ...
    public List<ProjectWaiver> getAllByResultFilter(final ProjectResultFilter projectResultFilter) {
        ...
        if(status!=null && status.size() > 0)
        {
            if(flag)
            {
                GET_ALL_PROJECT_WAIVERS_FILTER = GET_ALL_PROJECT_WAIVERS_FILTER + " where gw.status in ("+StringUtils.join(status, ',')+")";
                flag = false;
            }
            else
            {
                GET_ALL_PROJECT_WAIVERS_FILTER = GET_ALL_PROJECT_WAIVERS_FILTER + " and gw.status in ("+StringUtils.join(status, ',')+")";
            }
        }
    ...
    public Long getPendingActivityTotalCount(final User user, final ProjectResultFilter filter) {
        ...
        sqlBuilder.append("where (ga.approverid=? AND gw.status="+ SynchroGlobal.ProjectWaiverStatus.PENDING_APPROVAL.value()+")");
    ...
    public List<ProjectWaiver> getPendingApprovalWaivers(final User user, final ProjectResultFilter filter) {
        ...
        try {
            projectWaivers = getSimpleJdbcTemplate().getJdbcOperations().query(sqlBuilder.toString(), projectWaiverRowMapper, user.getID());
        } catch (DataAccessException e) {
            LOG.info(e.getMessage(), e);
        }
        return projectWaivers;
