...
@Decorate(false)
public class DashboardLogPaginationActionNew extends JiveActionSupport{

    private static final long serialVersionUID = 1L;
    private static final Logger LOGGER = Logger.getLogger(DashboardLogPaginationActionNew.class);
    ...
    private LogResultFilter getSearchFilter() {
        ...
        if(logResultFilter.getWaiverID() == null) {
        	logResultFilter.setWaiverID(waiverID);
        }
    ...
    public String downloadLogFile() throws IOException {
        ...
        header.append("Waiver Id").append(",");
            ...
            for(LogDashboardViewBean bean:logExtract) {
                if(bean.getDescriptions()!=null && bean.getDescriptions().size() > 0)
                {
                	for(String description : bean.getDescriptions())
                	{
                        ...
                        if(bean.getWaiverID() != null) {
                        	data.append("\"").append(bean.getWaiverID()).append("\"").append(",");
                        } else {
                            data.append(" ").append(",");
                        }
    ...
    public String downloadProjectLogFile() throws IOException {
        ...
        String waiverName = StringUtils.EMPTY;
        Long projectCode = getSearchFilter().getProjectID();
        Long waiverId = getSearchFilter().getWaiverID();
        ...
        {
        	Project project = synchroProjectManagerNew.get(projectCode);
        	if(project!=null)
        	{
        ...
        {
        	ProjectWaiver projectWaiver = projectWaiverManager.get(waiverId);
        	if(projectWaiver!=null)
        	{	
        		waiverName = projectWaiver.getName();
        		log.append(waiverName + " (" + SynchroUtils.generateProjectCode(waiverId)+")").append("\n");
        	}
        }
        else
        {
