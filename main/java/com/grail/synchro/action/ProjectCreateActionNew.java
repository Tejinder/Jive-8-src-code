 ...
 * @version 4.0
 */
public class ProjectCreateActionNew extends JiveActionSupport implements Preparable {
    ...
    public String execute() {
     ...
     //   project.setMultiMarket(false);
        ...
        try{
            ...
            if((SynchroPermHelper.isGlobalUserType() || SynchroPermHelper.isSystemAdmin() || SynchroPermHelper.isSynchroSystemOwner()) && globalProjectType!=null && globalProjectType.equals("EU"))
            {
            	...
            	// This is for preserving the correct URL for GLOBAL EU Projects on DRAFT dashboard
            	project.setProcessType(SynchroGlobal.ProjectProcessType.GLOBAL_EU_ONLINE.getId());
            }
            else if(SynchroPermHelper.isRegionalUserType() && globalProjectType!=null && globalProjectType.equals("EU"))
            {
            	project.setProjectType(SynchroGlobal.ProjectType.REGIONAL.getId());
         ...
         // This is done for setting the correct URL for GLOBAL projects in case they are saved multiple times
            else if(project.getProjectType()!=null && (project.getProjectType()==SynchroGlobal.ProjectType.GLOBAL.getId() || project.getProjectType()==SynchroGlobal.ProjectType.REGIONAL.getId()))
        	{
            	
        	}
        	
            else if(SynchroPermHelper.isEndMarketUserType() || SynchroPermHelper.isSystemAdmin() ||  SynchroPermHelper.isSynchroSystemOwner())
            {
            	project.setProjectType(SynchroGlobal.ProjectType.ENDMARKET.getId());
            }
            
            // As per new Requirements if the GlobalOutComeEUShare is No, then it will be treated as Global Non EU project.
            if(project.getGlobalOutcomeEUShare()!=null && project.getGlobalOutcomeEUShare().intValue()==2)
            {
            	project.setProcessType(SynchroGlobal.ProjectProcessType.GLOBAL_NON_EU.getId());
            }
            else if(project.getEndMarkets() != null && project.getEndMarkets().size()>0)
            {
            	...
            	else if(project.getProjectType()!=null && project.getEndMarkets().get(0)!=null)
            	{
            		project.setProcessType(SynchroUtils.getProjectProcessType(project.getProjectType(), project.getEndMarkets().get(0),project.getBudgetLocation()));
            	}
            }
        ...
        if(draft!=null && draft.contains("draft"))
        {
            if(isNewProject)
            {
            	if(globalProjectType!=null && globalProjectType.equals("EU"))
            	{
            		redirectUrl = "/new-synchro/create-project!input.jspa?projectID="+projectID+"&globalProjectType=EU&savefirst=true";
            	}
            	else
            	{
            		redirectUrl = "/new-synchro/create-project!input.jspa?projectID="+projectID+"&savefirst=true";
            	}
            ...
            {
            	// This is done for setting the correct URL for GLOBAL projects in case they are saved multiple times
            	if(project.getProjectType()!=null && (project.getProjectType()==SynchroGlobal.ProjectType.GLOBAL.getId() || project.getProjectType()==SynchroGlobal.ProjectType.REGIONAL.getId()))
            	{
            		redirectUrl = "/new-synchro/create-project!input.jspa?projectID="+projectID+"&globalProjectType=EU";
            	}
            	else
            	{
            		redirectUrl = "/new-synchro/create-project!input.jspa?projectID="+projectID;
            	}
