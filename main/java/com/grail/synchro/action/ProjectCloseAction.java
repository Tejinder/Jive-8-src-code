 ...
 */
public class ProjectCloseAction extends JiveActionSupport implements Preparable {

    private static final Logger LOG = Logger.getLogger(ProjectCloseAction.class);
    //Spring Managers
...
// To map the Methodology Waiver Fields
    ...
    public String input() {
    	...
    	if (SynchroPermHelper.hasProjectAccessNew1(projectID) ||  SynchroPermHelper.userTypeAccess(projectID) || SynchroPermHelper.isLegaUserType()) {
    		...
    		// Only System Admin or Synchro System Owner can access the Cancel Projects
    ...
    public String updateWaiver(){
      ...
      //Save Audit logs for change in Methodology Waiver related fields
        if(projectID!=null && endMarketDetails!=null && endMarketDetails.size()>0 && endMarketDetails.get(0).getEndMarketID()!=null)
        {
            ...
            SynchroLogUtils.PIBWaiverSave(pibMethodologyWaiver_DB, pibMethodologyWaiver, project, SynchroGlobal.LogProjectStage.PROJECT_CLOSE.getId());
        }
    ...
    public String resetProposal(){
	    ...
	    SynchroLogUtilsNew.addLog(SynchroGlobal.PortalType.SYNCHRO.getDescription(), SynchroGlobal.PageType.PROJECT.getId(), SynchroGlobal.Activity.NOTIFICATION.getId(), 
   				SynchroGlobal.LogProjectStage.PROJECT_CLOSE.getId(), getText("logger.project.proposal.reset"), project.getName(), 
   						project.getProjectID(), getUser().getID());
    ...
    public String resetPIB(){
    	...
    	SynchroLogUtilsNew.addLog(SynchroGlobal.PortalType.SYNCHRO.getDescription(), SynchroGlobal.PageType.PROJECT.getId(), SynchroGlobal.Activity.NOTIFICATION.getId(), 
				SynchroGlobal.LogProjectStage.PROJECT_CLOSE.getId(), getText("logger.project.pib.reset"), project.getName(), 
						project.getProjectID(), getUser().getID());
    	return "pib-details";
    }
  ...
  //Save/Update Funding End Market Details
    ...
    public String addAttachment() throws UnsupportedEncodingException {
        ...
        // This is done for keeping the Agency Waiver popup open in case Agency Waiver Attachment is added
        if(fieldCategoryId!=null && fieldCategoryId==SynchroGlobal.SynchroAttachmentObject.AGENCY_WAIVER.getId().intValue())
        {
        	return "agencyWaiver";
        }
        return SUCCESS;
