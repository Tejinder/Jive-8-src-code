...
@Decorate(false)
public class MethodologyWaiverPendingActivityPaginationActionNew extends JiveActionSupport {
...
//        }
        ...
        setPagination(pibManagerNew.getPIBPendingMethodologyWaiversTotalCount(filter).intValue());
        updatePage();
    ...
    {
    	downloadFilename = "PendingMethodologyWaiver.xls";
      ...
      //Audit Logs
        SynchroLogUtils.addLog(SynchroGlobal.PortalType.SYNCHRO.getDescription(), SynchroGlobal.PageType.REPORTS.getId(), SynchroGlobal.Activity.DOWNLOAD.getId(), 
				0, "Pending Methodology Waivers - Download Report", "", -1L, getUser().getID());
        
        return DOWNLOAD_REPORT;

    }
    
	public String approveMethodologyWaiver()
    {
	    ...
	    redirectUrl = "methodology-waiver-pending-activities.jspa?approve=true";
         return "methodology-waiver-pending-activities";
    }
    
    
    public String approveMultipleMethWaiver()
    {
    ...
    public String rejectMultipleMethWaiver()
    {
    	...
    	redirectUrl = "methodology-waiver-pending-activities.jspa?reject=true";
         return "methodology-waiver-pending-activities";
    }
    ...
    public void approveWaiver(Long projectID, String comment)
    {
    	...
    	if(comment!=null)
    	{
    		pibMethodologyWaiver.setMethodologyApproverComment(comment);
    	}
    ...
    public void updatePage() {
        ...
        pibMethodologyWaiverList = pibManagerNew.getPIBPendingMethodologyWaivers(getSearchFilter());
    ...
    private HSSFWorkbook generateReport(HSSFWorkbook workbook) throws UnsupportedEncodingException, IOException{
        ...
        generatedBy.append("\"").append("Filters:").append(Joiner.on(" | ").join(selectedFilters)).append("\"").append("\n");
         ...
         pibMethodologyWaiverList = pibManagerNew.getPIBPendingMethodologyWaivers(projectResultFilter);
         pibMethodologyWaiverList = toMethodologyWaiverBeans(pibMethodologyWaiverList);
	  ...
	  sheetHeaderFont.setFontName("Calibri");
         ...
         sheet.addMergedRegion(new CellRangeAddress(0,3,0,13));
         sheet.addMergedRegion(new CellRangeAddress(4,7,0,13));
         ...
         if(pibMethodologyWaiverList!=null && pibMethodologyWaiverList.size()>0)
         {
	         for(PIBMethodologyWaiver waiver : pibMethodologyWaiverList)
	         {
	         	...
	         	//sheet.addMergedRegion(new CellRangeAddress(startRow+5,startRow+8,0,13));
		  ...
		  notesStyle.setFillForegroundColor(IndexedColors.LEMON_CHIFFON.getIndex());
	        	...
	        	CellRangeAddress mergedRegion = new CellRangeAddress(startRow+3,startRow+8,0,13);
