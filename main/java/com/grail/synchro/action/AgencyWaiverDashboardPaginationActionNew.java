...
@Decorate(false)
public class AgencyWaiverDashboardPaginationActionNew extends JiveActionSupport {

    private static final long serialVersionUID = 1L;
    private static final Logger LOGGER = Logger.getLogger(AgencyWaiverDashboardPaginationActionNew.class);
...
//        }
        ...
        setPagination(pibManagerNew.getPIBAgencyWaiversTotalCount(filter).intValue());
        updatePage();
    ...
    public void updatePage() {
        ...
        pibMethodologyWaiverList = pibManagerNew.getPIBAgencyWaivers(getSearchFilter());
    ...
    {
    	downloadFilename = "AgencyWaiverDashboard.xls";
      ...
      //Audit Logs
        SynchroLogUtils.addLog(SynchroGlobal.PortalType.SYNCHRO.getDescription(), SynchroGlobal.PageType.REPORTS.getId(), SynchroGlobal.Activity.DOWNLOAD.getId(), 
				0, "Agency Waivers - Download Report", "", -1L, getUser().getID());
    ...
    private List<PIBMethodologyWaiver> toMethodologyWaiverBeans(final List<PIBMethodologyWaiver> methodologyWaivers) {
        ...
        if(methodologyWaivers != null && !methodologyWaivers.isEmpty()) 
        {
            for(PIBMethodologyWaiver methodologyWaiver : methodologyWaivers) 
            {
        		...
        		methodologyWaiver.setMethodology(SynchroDAOUtil.getMethodologyNames(StringUtils.join(methodologyWaiver.getMethodologyDetails(),",")));
	...
	private void processRawExtactReport() throws IOException {
        HSSFWorkbook workbook = null;
        workbook = new HSSFWorkbook();
        downloadFilename = "AgencyWaiverProjectsSummary.xls";
       ...
       generatedBy.append("Agency Waiver Projects Report ").append("\n");
       ...
       pibMethodologyWaiverList = pibManagerNew.getPIBAgencyWaivers(projectResultFilter);
       ...
       if(pibMethodologyWaiverList!=null && pibMethodologyWaiverList.size()>0)
       {
        ...
        workbook = SynchroRawExtractUtil.generateRawExtract(workbook,generatedBy.toString(),"AgencyWaiverProjects", projectIds);
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        workbook.write(baos);
        downloadStream = new ByteArrayInputStream(baos.toByteArray());
    ...
    private HSSFWorkbook generateReport(HSSFWorkbook workbook) throws UnsupportedEncodingException, IOException{
        ...
        StringBuilder generatedBy = new StringBuilder();
        generatedBy.append("Agency Waiver Report ").append("\n");
        ...
        generatedBy.append("\"").append("Filters:").append(Joiner.on(" | ").join(selectedFilters)).append("\"").append("\n");
         ...
         Integer startRow = 4;
         HSSFSheet sheet = workbook.createSheet("AgencyWaiverDashboard");
