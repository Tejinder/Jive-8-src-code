...
@Decorate(false)
public class ProjectFinancialPaginationReportAction extends JiveActionSupport{ 
	
	private static final long serialVersionUID = 1L;
	private static final Logger LOGGER = Logger.getLogger(ProjectFinancialPaginationReportAction.class);
...
//    	}
		...
		// Projects with Draft,Deleted and Canceled status will not be fetched.
    	ServletRequestDataBinder binder = new ServletRequestDataBinder(projectResultFilter);
        binder.bind(getRequest());
		projects = synchroProjectManager.getReportProjects(projectResultFilter);	
	...
	public List<ProjectStatusDashboardObject> getProjectPaginationObjectListBean(List<Project> projects)
    {
		...
		{
			//TODO Ignore Project with CANCEL and DELETE status as well
			//if(!SynchroGlobal.Status.getName(project.getStatus()).equalsIgnoreCase("DRAFT") && (SynchroPermHelper.hasProjectAccess(project.getProjectID()) || SynchroPermHelper.isSynchroAdmin(getUser())))
			if(SynchroPermHelper.hasProjectAccess(project.getProjectID()) || SynchroPermHelper.isSynchroAdmin(getUser()) || SynchroPermHelper.isSynchroMiniAdmin()) 
			{
				...
				try{
						...
						//Getting Ebd MarketList for project 
						List<Long> endMarkets = synchroProjectManager.getEndMarketIDs(project.getProjectID());
						//coordinationDetailsManager.get
						List<Integer> coSupplierGroups = coordinationDetailsManager.getCoordinationSupplierGroupList(project.getProjectID());
						List<Integer> fwSupplierGroups = coordinationDetailsManager.getFieldWorkSupplierGroupList(project.getProjectID());
						...
						List<String> supplierGroups = SynchroUtils.getUniqueSupplierList(coSupplierGroups, fwSupplierGroups);
						...
						projectStatusDashboardObject.setStatus(SynchroGlobal.Status.getName(project.getStatus()));
