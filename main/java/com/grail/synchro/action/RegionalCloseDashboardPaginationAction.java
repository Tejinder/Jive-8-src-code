...
@Decorate(false)
public class RegionalCloseDashboardPaginationAction extends JiveActionSupport{

    private static final long serialVersionUID = 1L;
    private static final Logger LOGGER = Logger.getLogger(RegionalCloseDashboardPaginationAction.class);
...
//        }
    ...
    {
    	downloadFilename = "RegionalCloseProjectsDashboard.xls";
        ...
        try
        {
        	if(downloadReportType!=null && downloadReportType.equals("ProjectSummary"))
        	{
                ...
                SynchroLogUtils.addLog(SynchroGlobal.PortalType.SYNCHRO.getDescription(), SynchroGlobal.PageType.REPORTS.getId(), SynchroGlobal.Activity.DOWNLOAD.getId(), 
        				0, "Regional Close Projects Summary - Download Report", "", -1L, getUser().getID());
        	}
        	else
        	{
        		processReport();
        		//Audit Logs
                SynchroLogUtils.addLog(SynchroGlobal.PortalType.SYNCHRO.getDescription(), SynchroGlobal.PageType.REPORTS.getId(), SynchroGlobal.Activity.DOWNLOAD.getId(), 
        				0, "Regional Close Projects Status - Download Report", "", -1L, getUser().getID());
        	}
        }
        catch(Exception e)
        {
    ...
    private void processReport() throws IOException {
     ...
     private void processRawExtactReport() throws IOException {
        HSSFWorkbook workbook = null;
        workbook = new HSSFWorkbook();
        downloadFilename = "RegionalCloseProjectsSummary.xls";
       ...
       generatedBy.append("Regional Close Projects Report ").append("\n");
       ...
       if(projects!=null && projects.size()>0)
       {
        ...
        workbook = SynchroRawExtractUtil.generateRawExtract(workbook,generatedBy.toString(),"RegionalCloseProjects", projectIds);
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        workbook.write(baos);
        downloadStream = new ByteArrayInputStream(baos.toByteArray());
    ...
    private HSSFWorkbook generateReport(HSSFWorkbook workbook) throws UnsupportedEncodingException, IOException{
        ...
        generatedBy.append("\"").append("Filters:").append(Joiner.on(" | ").join(selectedFilters)).append("\"").append("\n");
         ...
         Integer startRow = 4;
         HSSFSheet sheet = workbook.createSheet("RegionalCloseProjectsDashboard");
