...
public class ShareFileDashboardAction extends JiveActionSupport {
    ...
    public String execute() {
       ...
       // System.out.println("TILE ID ==>"+ tileID);
		...
		ospFolder = ospManager.getShareFolder(tileID, folderID);
    	return SUCCESS;
    }
    ...
    public String addAttachment() throws UnsupportedEncodingException {
        ...
        {        	
            ...
            if(ospManager.isValidShareFileName(ospFile))
            {
            	if(attachFile!=null)
            	{
	            	Attachment attachment =  ospManager.addAttachment(attachFile, attachFileFileName, attachFileContentType, tileID, folderID);
		        	 ...
		        	 if(attachment.getSize()>0)
		        	 {
			             ...
			             ospManager.saveShareFile(ospFile);
			             
			             List<OSPFile> ospFileList = ospManager.getShareFiles(ospFile.getTileId(), ospFile.getFolderId());
			 		     ...
			 		     ospManager.updateShareFolderSize(ospFolder);
			 		     redirectURL="share-file-dashboard.jspa?tileID="+tileID+"&folderID="+folderID;
		        	 }
		        	 else
		        	 {
		        		// redirectURL="share-file-dashboard.jspa?tileID="+tileID+"&folderID="+folderID+"&validFileSize=false&fileName="+attachFileFileName;
		        		 redirectURL="share-file-dashboard.jspa?tileID="+tileID+"&folderID="+folderID+"&validFileSize=false";
		        	 }
            	}
            	else
            	{
            ...
            {
            	//redirectURL="share-file-dashboard.jspa?tileID="+tileID+"&folderID="+folderID+"&validFile=false&fileName="+attachFileFileName;
            	redirectURL="share-file-dashboard.jspa?tileID="+tileID+"&folderID="+folderID+"&validFile=false";
            }
	...
	public String delete() {
    	...
    	if(StringUtils.isNotBlank(deleteFileIDs))
    	{
	    	if(deleteFileIDs.contains(","))
	    	{
		    	...
		    	{	
		    		ospManager.deleteShareFile(new Long(dFileId));
		    	}
	    	}
	    	else
	    	{
	    		ospManager.deleteShareFile(new Long(deleteFileIDs));
	    	}
	    	
	    	
	    	List<OSPFile> ospFileList = ospManager.getShareFiles(tileID, folderID);
			 Long folderFileSize = new Long("0");
			 for(OSPFile oFile:ospFileList )
			 {
				folderFileSize = folderFileSize + oFile.getFileSize();
