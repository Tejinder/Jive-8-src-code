...
public class ShareFolderDashboardAction extends JiveActionSupport {
    ...
    public String createNewFolder() {
        if(!SynchroPermHelper.canEditOSPSharePortal(getUser())) {
            return UNAUTHORIZED;
        }
       ...
       // ospFolder.setFolderName(folderName);
        ...
        if(ospManager.isValidShareFolderName(ospFolder))
        {
        	ospManager.saveShareFolder(ospFolder);
        	redirectURL="share-folder-dashboard.jspa?tileID="+tileID;
        }
        else
        {
        	redirectURL="share-folder-dashboard.jspa?tileID="+tileID+"&validFolder=false&folderName="+folderName;
        }
    ...
    public String renameFolder() {
       ...
       // ospFolder.setFolderName(renameFolderName);
        ...
        if(ospManager.isValidShareFolderName(ospFolder))
        {
        	ospManager.updateShareFolderName(ospFolder);
        	redirectURL="share-folder-dashboard.jspa?tileID="+tileID;
        }
        else
        {
        	redirectURL="share-folder-dashboard.jspa?tileID="+tileID+"&validFolder=false&folderName="+renameFolderName;
        }
    ...
    public String delete() {
    	...
    	if(StringUtils.isNotBlank(deleteFolderIDs))
    	{
	    	if(deleteFolderIDs.contains(","))
	    	{
		    	...
		    	{	
		    		ospManager.deleteShareFolder(new Long(dFolderId));
		    	}
	    	}
	    	else
	    	{
	    		ospManager.deleteShareFolder(new Long(deleteFolderIDs));
	    	}
    	}
