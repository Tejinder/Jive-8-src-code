 ...
 * @since: 1.0
 */
public class ModifyProjectReportSummaryTask4 implements UpgradeTask {
	
	private static final Logger LOGGER = Logger.getLogger(ModifyProjectReportSummaryTask3.class);
	
	
	private static String ADD_COL1 = "ALTER TABLE grailprojectrepsummary ADD islegalsignoffreq integer";
    ...
    public void doTask() throws Exception {
        if (UpgradeUtils.doesTableExist("grailprojectrepsummary")) {
        	...
        	try{
	        	...
	        	}catch(Exception e){LOGGER.error("Error while updating grailprojectrepsummary table - adding new columns - islegalsignoffreq" + e.getMessage());}
        }
    }
}
